...
class UCL_Device {
 public:
  ...
  inline void push_command_queue() {
    ...
    _cq.push_back(UPTKStream_t());
    UPTKStreamCreateD(&_cq.back(),0);
  }
 ...
 private:
  ...
  std::vector<UPTKStream_t> _cq;
  UPTKdevice _cu_device;
  UPTKcontext _context;
#if GERYON_NVD_PRIMARY_CONTEXT
  UPTKcontext _old_context;
#endif
};
...
#error CUDA Toolkit version 8 or later required
  ...
  UPTKDeviceGetCountD(&_num_devices);
  for (int i=0; i<_num_devices; ++i) {
    UPTKdevice dev;
    //CUdevice dev;
    //CU_SAFE_CALL_NS(cuDeviceGet(&dev,i));
    UPTKDeviceGetD(&dev,i);
    int major, minor;
    ...
    /*UPTKDeviceGetAttributeD(&major, CU_DEVICE_ATTRIBUTE_COMPUTE_CAPABILITY_MAJOR, dev);
    UPTKDeviceGetAttributeD(&minor, CU_DEVICE_ATTRIBUTE_COMPUTE_CAPABILITY_MINOR, dev);*/
    UPTKDeviceGetAttributeD(&major, UPTKDevAttrComputeCapabilityMajor, dev);
    UPTKDeviceGetAttributeD(&minor, UPTKDevAttrComputeCapabilityMinor, dev);
    if (major==9999)
      continue;
    ...
    UPTKDeviceGetNameD(namecstr,1024,dev);
    prop.name=namecstr;

    //CU_SAFE_CALL_NS(cuDeviceTotalMem(&prop.totalGlobalMem,dev));
    UPTKDeviceTotalMem_v2D(&prop.totalGlobalMem,dev);
    ...
    UPTKDeviceGetAttributeD(&prop.multiProcessorCount, UPTKDevAttrMultiProcessorCount, dev);
    UPTKDeviceGetAttributeD(&prop.maxThreadsPerBlock, UPTKDevAttrMaxThreadsPerBlock, dev);
    UPTKDeviceGetAttributeD(&prop.maxThreadsDim[0], UPTKDevAttrMaxBlockDimX, dev);
    UPTKDeviceGetAttributeD(&prop.maxThreadsDim[1], UPTKDevAttrMaxBlockDimY, dev);
    UPTKDeviceGetAttributeD(&prop.maxThreadsDim[2], UPTKDevAttrMaxBlockDimZ, dev);
    UPTKDeviceGetAttributeD(&prop.maxGridSize[0], UPTKDevAttrMaxGridDimX, dev);
    UPTKDeviceGetAttributeD(&prop.maxGridSize[1], UPTKDevAttrMaxGridDimY, dev);
    UPTKDeviceGetAttributeD(&prop.maxGridSize[2], UPTKDevAttrMaxGridDimZ, dev);
    UPTKDeviceGetAttributeD(&prop.sharedMemPerBlock, UPTKDevAttrMaxSharedMemoryPerBlock, dev);
    UPTKDeviceGetAttributeD(&prop.totalConstantMemory, UPTKDevAttrTotalConstantMemory, dev);
    UPTKDeviceGetAttributeD(&prop.SIMDWidth, UPTKDevAttrWarpSize, dev);
    UPTKDeviceGetAttributeD(&prop.memPitch, UPTKDevAttrMaxPitch, dev);
    UPTKDeviceGetAttributeD(&prop.regsPerBlock, UPTKDevAttrMaxSharedMemoryPerBlock, dev);//cudamoker zhushi
    UPTKDeviceGetAttributeD(&prop.clockRate, UPTKDevAttrClockRate, dev);
    UPTKDeviceGetAttributeD(&prop.textureAlign, UPTKDevAttrTextureAlignment, dev);
    UPTKDeviceGetAttributeD(&prop.kernelExecTimeoutEnabled, UPTKDevAttrKernelExecTimeout,dev);
    UPTKDeviceGetAttributeD(&prop.integrated, UPTKDevAttrIntegrated, dev);
    UPTKDeviceGetAttributeD(&prop.canMapHostMemory, UPTKDevAttrCanMapHostMemory, dev);
    UPTKDeviceGetAttributeD(&prop.computeMode, UPTKDevAttrComputeMode,dev);
    UPTKDeviceGetAttributeD(&prop.concurrentKernels, UPTKDevAttrConcurrentKernels, dev);
    UPTKDeviceGetAttributeD(&prop.ECCEnabled, UPTKDevAttrEccEnabled, dev);
...
int UCL_Device::set(int num) {
  ...
  /*cuCtxGetCurrent(&_old_context);
  cuDeviceGet(&_cu_device,_device);
  CUresult err=cuDevicePrimaryCtxRetain(&_context,_cu_device);*/

  UPTKCtxGetCurrentD(&_old_context);
  UPTKDeviceGetD(&_cu_device,_device);
  UPTKError err=UPTKDevicePrimaryCtxRetainD(&_context,_cu_device);
#else
  CU_SAFE_CALL_NS(cuDeviceGet(&_cu_device,_device));
...
void UCL_Device::clear() {
  if (_device>-1) {
    ...
    UPTKCtxSetCurrentD(_old_context);
    UPTKDevicePrimaryCtxReleaseD(_cu_device);
#else
...
void UCL_Device::print_all(std::ostream &out) {
  ...
  UPTKDriverGetVersionD(&driver_version);
