...
#error CUDA Toolkit version 8 or later required
  ...
  for (int i=0; i<_num_devices; ++i) {
    ...
    UPDeviceGetAttribute(&major, UPTKDevAttrComputeCapabilityMajor, dev);
    UPDeviceGetAttribute(&minor, UPTKDevAttrComputeCapabilityMinor, dev);
    if (major==9999)
      continue;
    ...
    UPDeviceGetName(namecstr,1024,dev);
    prop.name=namecstr;

    //CU_SAFE_CALL_NS(cuDeviceTotalMem(&prop.totalGlobalMem,dev));
    UPDeviceTotalMem_v2(&prop.totalGlobalMem,dev);
    ...
    UPDeviceGetAttribute(&prop.multiProcessorCount, UPTKDevAttrMultiProcessorCount, dev);
    UPDeviceGetAttribute(&prop.maxThreadsPerBlock, UPTKDevAttrMaxThreadsPerBlock, dev);
    UPDeviceGetAttribute(&prop.maxThreadsDim[0], UPTKDevAttrMaxBlockDimX, dev);
    UPDeviceGetAttribute(&prop.maxThreadsDim[1], UPTKDevAttrMaxBlockDimY, dev);
    UPDeviceGetAttribute(&prop.maxThreadsDim[2], UPTKDevAttrMaxBlockDimZ, dev);
    UPDeviceGetAttribute(&prop.maxGridSize[0], UPTKDevAttrMaxGridDimX, dev);
    UPDeviceGetAttribute(&prop.maxGridSize[1], UPTKDevAttrMaxGridDimY, dev);
    UPDeviceGetAttribute(&prop.maxGridSize[2], UPTKDevAttrMaxGridDimZ, dev);
    UPDeviceGetAttribute(&prop.sharedMemPerBlock, UPTKDevAttrMaxSharedMemoryPerBlock, dev);
    UPDeviceGetAttribute(&prop.totalConstantMemory, UPTKDevAttrTotalConstantMemory, dev);
    UPDeviceGetAttribute(&prop.SIMDWidth, UPTKDevAttrWarpSize, dev);
    UPDeviceGetAttribute(&prop.memPitch, UPTKDevAttrMaxPitch, dev);
    UPDeviceGetAttribute(&prop.regsPerBlock, UPTKDevAttrMaxSharedMemoryPerBlock, dev);//cudamoker zhushi
    UPDeviceGetAttribute(&prop.clockRate, UPTKDevAttrClockRate, dev);
    UPDeviceGetAttribute(&prop.textureAlign, UPTKDevAttrTextureAlignment, dev);
    UPDeviceGetAttribute(&prop.kernelExecTimeoutEnabled, UPTKDevAttrKernelExecTimeout,dev);
    UPDeviceGetAttribute(&prop.integrated, UPTKDevAttrIntegrated, dev);
    UPDeviceGetAttribute(&prop.canMapHostMemory, UPTKDevAttrCanMapHostMemory, dev);
    UPDeviceGetAttribute(&prop.computeMode, UPTKDevAttrComputeMode,dev);
    UPDeviceGetAttribute(&prop.concurrentKernels, UPTKDevAttrConcurrentKernels, dev);
    UPDeviceGetAttribute(&prop.ECCEnabled, UPTKDevAttrEccEnabled, dev);
...
int UCL_Device::set(int num) {
  ...
  UPCtxGetCurrent(&_old_context);
  UPDeviceGet(&_cu_device,_device);
  UPTKError err=UPDevicePrimaryCtxRetain(&_context,_cu_device);
#else
  CU_SAFE_CALL_NS(cuDeviceGet(&_cu_device,_device));
...
void UCL_Device::clear() {
  if (_device>-1) {
    ...
    UPCtxSetCurrent(_old_context);
    UPDevicePrimaryCtxRelease(_cu_device);
#else
...
void UCL_Device::print_all(std::ostream &out) {
  ...
  UPDriverGetVersion(&driver_version);
