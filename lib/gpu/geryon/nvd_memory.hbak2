...
inline int _host_alloc(mat_type &mat, copy_type &cm, const size_t n,
  ...
  if (kind==UCL_NOT_PINNED) {
    *(mat.host_ptr())=(typename mat_type::data_type*)malloc(n);
    ...
    err=UPTKMemHostAllocD((void **)mat.host_ptr(),n,CU_MEMHOSTALLOC_WRITECOMBINED);
  }
  else {
    //err=cuMemAllocHost((void **)mat.host_ptr(),n);
    err=UPTKMemAllocHostD((void **)mat.host_ptr(),n);
  }
...
inline void _device_zero(mat_type &mat, const size_t n, command_queue &cq) {
  ...
  if (n%32==0)
    UPTKMemsetD32AsyncD(mat.cbegin(),0,n/4,cq);
  else if (n%16==0)
    UPTKMemsetD16AsyncD(mat.cbegin(),0,n/2,cq);
  else
    UPTKMemsetD8AsyncD(mat.cbegin(),0,n,cq);
}
